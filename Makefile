...
build/envs.txt: build/.setup_conda_envs.make_marker
	@mkdir -p build/
	conda env list | grep myenv | rev | cut -d " " -f1 | rev > build/envs.txt.tmp
...
BDIST_WHEEL_FILE = $(shell bash -c "ls -1t dist/*py*.whl | head -n 1")
SDIST_FILE = $(shell bash -c "ls -1t dist/*.tar.gz | head -n 1")
...
build/.src_files.txt: setup.py build/envs.txt src/*/*.py
	@mkdir -p build/
	@ls -l setup.py build/envs.txt src/*/*.py > build/.src_files.txt.tmp
...
build: build/.local_install.make_marker
	...
	@$(PYTHON37) setup.py bdist_wheel --python-tag=py36.py37 >> $(BUILD_LOG_FILE)
	@echo "build completed for $(LIB_NAME)"


publish: build/.install.make_marker build/README.html
	$(PYENV37)/bin/pycalver bump
	$(PYTHON37) setup.py bdist_wheel --python-tag=py36.py37
...
run: build/.install.make_marker
	PYTHONPATH=src/:$$PYTHONPATH $(PYTHON37) -m $(LIB_NAME) --help
